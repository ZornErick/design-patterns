...
### Aplicabilidade
Use o padrão Observer em qualquer das seguintes situações:
* quando uma abstração tem dois aspectos, um dependente do outro. Encapsular esses aspectos em objetos separados permite variá-los e reutilizá-los independentemente.
* quando uma mudança em um objeto exige mudanças em outros, e você não sabe quantos objetos necessitam ser mudados.
* quando um objeto deveria ser capaz de notificar outros objetos sem fazer hipóteses sobre quem são esses objetos. Em outras palavras, você não quer esses objetos fortemente acoplados.

### Consequências
O padrão Observer permite variar subjects e observers independentemente. Você pode reutilizar subjects sem reutilizar seus observers, e vice-versa. Ele lhe permite acrescentar observers sem modificar o subject ou outros observers. Outros benefícios e desvantagens:
1. Acoplamento abstrato entre Subject e Observer. Tudo o que um subject conhece é que tem uma lista de observers, cada um deles correspondendo à interface da classe Observer simples. O subject não conhece a classe concreta de qualquer observer. Dessa forma, o acoplamento entre subjects e observers é abstrato e mínimo. Como subject e observer não estão fortemente acoplados, eles podem pertencer a diferentes camadas de abstração de um sistema.
2. Suporte para a comunicação por difusão (broadcast). Diferentemente de uma solicitação ordinária, a notificação que um subject envia não necessita especificar seu receptor. A notificação é difundida automaticamente a todos os objetos interessados que tiveram registrar o seu interesse.
3. Atualizações inesperadas. Como observers não têm conhecimento da presença de outros observers, eles podem ser indiferentes ao custo final de mudanças no subject. Uma operação aparentemente inócua sobre o subject pode causar uma cascata de atualizações nos observers e em seus objetos dependentes. Além disso, critérios de dependência que não estão bem definidos ou mantidos comumente conduzem a atualizações espúrias, que podem ser difíceis de depurar.

### Implementação
Vários tópicos relacionados à implementação do padrão Observer são discutidos a seguir:
1. Mapeando subjects para observers. A maneira mais simples de um subject manter o registro dos observers que devem ser notificados é armazenar referências para eles explicitamente no subject. No entanto, esse armazenamento pode ser muito caro quando há muitos subjects e poucos observers. Uma solução é compensar o gasto com tempo de armazenamento mantendo uma pesquisa associativa (por exemplo, uma tabela randômica) para manter o mapeamento subject-para-observer. Assim, um subject sem observers não incorre em sobrecarga de armazenamento. Por outro lado, esta abordagem aumenta o custo de acessar os observers.
2. Observando mais de um subject. Em alguns casos, pode fazer sentido para um observer depender de mais de um subject. Por exemplo, uma planilha pode depender de mais de uma fonte de dados. É necessário estender a interface Update em tais casos para deixar o observer saber que subject está enviando a notificação. O subject pode simplesmente passar a si mesmo como um parâmetro na operação Update, dessa forma deixando o observer saber qual subject examinar.
3. Quem dispara a atualização? O subject e seus observers contam com o mecanismo de notificação para se manterem consistentes. Mas que objeto realmente chama Notify para acionar a atualização? Aqui estão duas opções: (a) Faça com que as operações que mudem o estado em Subject chamem Notify após a mudança de estado. A vantagem é que os clientes não têm que se lembrar de chamar Notify no subject. A desvantagem é que várias operações consecutivas causarão várias atualizações sucessivas, o que pode ser ineficiente. (b) Tornar os clientes responsáveis pela chamada à operação Notify no momento certo. A vantagem é que o cliente pode esperar para disparar a atualização até depois que uma série de mudanças de estado tenham sido feitas, evitando dessa forma atualizações intermediárias desnecessárias. A desvantagem é que os clientes têm uma responsabilidade extra de disparar a atualização. Isso aumenta a probabilidade de erros, uma vez que os clientes podem esquecer de chamar Notify.
4. Referências pendentes para subjects deletados. Apagar um subject não deveria produzir referências pendentes em seus observers. Uma maneira de evitar referências pendentes é fazer com que o subject notifique seus observers à medida que é deletado para que possam reiniciar suas referências para ele.
5. Garantia de estado de Subject autoconsistente antes da notificação. É importante garantir que o estado do Subject é autoconsistente antes de chamar Notify, porque os observers consultam o estado do subject no curso da atualização do seu próprio estado.
6. Evitando protocolos de atualização específicos do observer: os modelos push e pull. As implementações do padrão Observer fazem frequentemente o subject difundir informação adicional sobre a mudança. O subject passa essa informação como argumento para Update. A quantidade de informação pode variar consideravelmente. Em um extremo, que chamamos de modelo push, o subject envia aos observers informação detalhada sobre a mudança, querendo eles ou não. No outro extremo está o modelo pull; o subject não envia mais do que a notificação mínima, e os observers solicitam detalhes explicitamente após isso.
7. Especificando explicitamente modificações de interesse. Você pode melhorar a eficiência da atualização estendendo o registro de interesse do subject para suportar registro de eventos específicos de interesse. Quando um evento dessa natureza ocorre, o subject informa somente os observers que registraram interesse naquele evento. Uma forma de suportar isso utiliza a noção de aspectos para os objetos Subject.
8. Encapsulando semânticas de atualização complexas. Quando o relacionamento de dependência entre subjects e observers é particularmente complexo, pode ser necessário um objeto que mantenha esses relacionamentos. Chamamos tal objeto de ChangeManager. Seu propósito é minimizar o trabalho exigido para fazer com que os observers reflitam uma mudança em seu subject.
9. Combinando as classes Subject e Observer. Bibliotecas escritas em linguagens que carecem de herança múltipla (como Smalltalk) geralmente não definem classes Subject e Observer separadas, mas combinam suas interfaces em uma classe. Isso lhe permite definir um objeto que atua como um subject e um observer sem herança múltipla.

## State
### Objetivo
Permitir a um objeto alterar seu comportamento quando o seu estado interno muda. O objeto parecerá ter mudado sua classe.

### Aplicabilidade
Use o padrão State em qualquer dos seguintes casos:
...
* operações têm comandos condicionais grandes, de várias alternativas, que dependem do estado do objeto. Esse estado é normalmente representado por uma ou mais constantes enumeradas. Frequentemente, várias operações conterão essa mesma estrutura condicional. O padrão State coloca cada ramo do comando condicional em uma classe separada. Isso lhe permite tratar o estado do objeto, propriamente dito, como um objeto que pode variar independentemente de outros objetos.

### Consequências
O padrão State tem as seguintes consequências:
1. Localiza comportamento específico de estado e particiona comportamento para estados diferentes. O padrão State coloca todo o comportamento associado a um estado particular em um objeto. Visto que todo o código específico do estado está em uma subclasse State, novos estados e transições podem ser facilmente acrescentados pela definição de novas subclasses.
2. Torna explícitas as transições de estados. Quando um objeto define seu estado corrente somente em termos de valores internos de dados, suas transições de estado não têm uma representação explícita; elas só aparecem como atribuições para algumas variáveis. Introduzir objetos separados para diferentes estados torna as transições mais explícitas. Além disso, os objetos State podem proteger Context contra estados internos inconsistentes, porque transições de estado são, do ponto de vista de Context, operações atômicas.
3. Os objetos State podem ser compartilhados. Se os objetos State não tiverem variáveis de instância — ou seja, o estado que eles representam é codificado inteiramente no seu tipo —, então os objetos contextos podem compartilhar um objeto State. Quando estados são compartilhados desta forma, eles essencialmente são flyweights sem nenhum estado intrínseco, somente comportamento.

### Implementação
O padrão State levanta uma série de tópicos a serem considerados na implementação:
1. Quem define as transições de estado? O padrão State não especifica que participante define o critério para transições de estado. Se o critério é fixo, então pode ser implementado totalmente no Context. É geralmente mais flexível e apropriado, contudo, deixar que as próprias subclasses ConcreteState especifiquem o seu estado sucessor e quando fazer a transição.
...
## Template Method
### Objetivo
Definir o esqueleto de um algoritmo em uma operação, postergando a definição de alguns passos para subclasses. Template Method permite que subclasses redefinam certos passos de um algoritmo sem mudar a estrutura do mesmo.

### Aplicabilidade
O padrão Template Method deveria ser usado:
...
* quando o comportamento comum entre as subclasses deveria ser fatorado e concentrado em uma classe comum para evitar a duplicação de código. Esse é um bom exemplo de "refactoring para generalização" (refactoring to generalize), descrito por Opdyke e Johnson. Você primeiro identifica as diferenças no código existente e então separa as diferenças em novas operações. Por fim, você substitui o código diferente por um método-template que chama uma dessas novas operações;
* para controlar extensões em subclasses. Você pode definir um método-template que chama operações "gancho" (hook operations) em pontos específicos, dessa forma permitindo extensões somente naqueles pontos.

### Consequências
Os métodos-templates são uma técnica fundamental para a reutilização de código. Eles são particularmente importantes em bibliotecas de classes, porque são o meio para fatorizar o comportamento comum em bibliotecas de classes. Os métodos-templates conduzem a uma estrutura invertida de controle algumas vezes referenciada como "o princípio de Hollywood", ou seja, "Não nos chame, nós chamaremos você". Isto se refere ao fato de que uma classe-mãe chama as operações de uma subclasse e não o contrário. Os métodos-template chamam os seguintes tipos de operações: operações concretas (sobre a ConcreteClass ou sobre as classes do cliente); operações de AbstractClass (operações que são geralmente úteis para as subclasses); operações primitivas (operações abstratas); operações fábricas; e operações gancho, as quais fornecem comportamento default que as subclasses podem estender, se necessário. É importante para um método-template especificar quais operações são ganchos (que podem ser redefinidos) e quais são as operações abstratas (que devem ser redefinidas).

### Implementação
Há três tópicos de implementação que valem a pena destacar:
1. Utilizando controle de acesso C++. Em C++, as operações primitivas que um método-template chama podem ser declaradas como funções membros protegidas. Isso garante que sejam chamadas somente pelo método-template. Operações primitivas que devem ser redefinidas são declaradas como funções membro virtuais puras. O próprio método-template não deveria ser redefinido; portanto, você pode tornar o método-template uma função membro não-virtual.
2. Minimizando operações primitivas. Um objetivo importante para o projeto de métodos-templates é minimizar o número de operações primitivas que uma subclasse deve redefinir para concretizar o algoritmo. Quanto mais operações precisarem ser redefinidas, mais tedioso fica para os clientes.
3. Convenções de nomenclatura. Você pode identificar as operações que devem ser redefinidas adicionando um prefixo aos seus nomes. Por exemplo, o framework MacApp para aplicações Macintosh acrescenta o prefixo "Do-": "DoCreateDocument", "DoRead" e assim por diante.
...
* uma estrutura de objetos contém muitas classes de objetos com diferentes interfaces e você quer executar operações sobre esses objetos que dependem de suas classes concretas;
* muitas operações distintas e não relacionadas necessitam ser executadas sobre objetos em uma estrutura de objetos, e você quer evitar "poluição" das classes desses objetos com tais operações. O Visitor permite manter juntas operações relacionadas, definindo-as em uma classe. Quando a estrutura de objetos é compartilhada por muitas aplicações, use o padrão Visitor para colocar as operações somente naquelas aplicações que precisem delas;
* as classes que definem a estrutura de objetos raramente mudam, mas você frequentemente quer definir novas operações sobre a estrutura. A mudança das classes da estrutura de objetos exige redefinir a interface para todos os visitantes, o que é potencialmente custoso. Se as classes da estrutura de objetos mudam frequentemente, então é provavelmente melhor definir as operações nessas classes.

### Consequências
Algumas das consequências do padrão Visitor são as seguintes:
1. Visitor torna fácil adicionar novas operações. Os visitantes tornam fácil acrescentar operações que dependem dos componentes de objetos complexos. Você pode definir uma nova operação sobre uma estrutura de objetos simplesmente acrescentando um novo visitante. Em comparação, se você espalhar a funcionalidade através de muitas classes, então tem que mudar cada classe para definir uma nova operação.
2. Um visitante reúne operações relacionadas e separa as não-relacionadas. Comportamentos relacionados não estão espalhados sobre as classes que definem a estrutura de objetos; eles estão localizados em um visitante. Comportamentos não relacionados são particionados em suas próprias subclasses Visitor.
3. Acrescentar novas classes ConcreteElement é difícil. O padrão Visitor torna difícil acrescentar novas subclasses do Element. Cada novo ConcreteElement requer uma nova operação abstrata em Visitor e a correspondente implementação em cada classe ConcreteVisitor. Algumas vezes, uma implementação default pode ser fornecida em Visitor que pode ser herdada pela maioria dos ConcreteVisitors, mas isto é mais a exceção do que a regra. Por isso a chave para se considerar quando se aplica o padrão Visitor é se você está mais propenso a mudar o algoritmo aplicado sobre uma estrutura de objetos ou a mudar as classes de objetos que fazem parte da estrutura.
4. Percorrendo as hierarquias de classes. Um iterador pode percorrer uma estrutura de objetos chamando suas operações. Mas um iterador não pode trabalhar em estruturas de objetos com diferentes tipos de elementos.
5. Acumulando estado. Os visitantes podem acumular estado à medida que visitam cada elemento na estrutura de objetos. Sem um visitante, esse estado seria passado como argumentos extras para as operações que executam o percurso, ou eles podem aparecer como variáveis globais.
6. Quebrando o encapsulamento. A abordagem do padrão Visitor presume que a interface ConcreteElement é poderosa o suficiente para deixar visitantes fazerem seu trabalho. Como resultado, o padrão geralmente força a fornecer operações públicas que acessam o estado interno de um elemento, o que pode comprometer seu encapsulamento.

### Implementação
Cada estrutura de objetos terá uma classe Visitor associada. Esta classe abstrata Visitor declara uma operação `VisitConcreteElement` para cada classe de ConcreteElement que define a estrutura de objetos. Cada operação Visit em Visitor declara seu argumento como uma classe ConcreteElement particular, permitindo a Visitor acessar a interface de ConcreteElement diretamente. As classes ConcreteVisitor redefinem cada operação Visit para implementar comportamento específico de visitantes para a ConcreteElement correspondente. Aqui estão dois outros tópicos de implementação que aparecem quando aplica-se o padrão Visitor:
1. Double-dispatch. Efetivamente, o padrão Visitor permite acrescentar operações às classes sem mudá-las. O Visitor consegue isso usando uma técnica chamada double-dispatch. É uma técnica bem conhecida. De fato, algumas linguagens de programação a suportam diretamente (por exemplo, CLOS). Linguagens como C++ e Smalltalk suportam single-dispatch. Em linguagens single-dispatch, duas critérios determinam que operação satisfará uma solicitação: o nome da solicitação e o tipo do receptor. Por exemplo, a operação que uma chamada GenerateCode irá chamar depende do tipo do nó que você pediu. Em C++, chamar GenerateCode em uma instância de VariableRefNode chamará VariableRefNode::GenerateCode (que gera código para uma referência a variável). Esta declaração da operação em duas classes envia a chamada para diferentes operações. Double-dispatch significa simplesmente que a operação que é executada depende do tipo de duas solicitações: a solicitação e os tipos de dois receptores. Accept é uma operação double-dispatch. Seu significado depende de duas tipos: o tipo de Visitor e o tipo de Element. Double-dispatching permite que visitantes solicitem operações diferentes em cada classe de Element. Esta é a chave para o padrão Visitor: a operação que é executada depende tanto do tipo do Visitor como do tipo do Element que ele visita.
2. Quem é responsável por percorrer a estrutura de objetos? Um visitante deve visitar cada elemento de uma estrutura de objetos. A questão é, como ele chega lá? Podemos colocar a responsabilidade pelo percurso em qualquer dos três lugares: na estrutura de objetos, no visitante ou em um objeto iterador separado. Frequentemente a estrutura de objetos é responsável pela iteração. Uma coleção simplesmente irá iterar sobre seus elementos, chamando a operação Accept em cada um. Um composite irá comumente percorrer-se fazendo com que cada operação Accept percorra os filhos do elemento e chame Accept recursivamente em cada um deles.
